...
object Csv:
  ...
  def make[F[_]: Logger: Files: RaiseThrowable: Async](
      filePath: String,
    ...
    Async[F].pure(
      new Csv[F]:
        override def read(): F[Stream[F, Map[String, String]]] =
          for _ <- Logger[F].info(s"Reading data from $filePath...")
          yield Files[F]
            ...
            .map(row =>
              (if includeLineNumber then row.set(LineNumberColumn, row.line.map(_.toString).getOrElse(""))
               else row).toMap
            )
    )
